#!/usr/bin/env python3
"""
Raw capture evaluation: Run inference on N datasets and save FULL outputs.
No truncation, no complex parsing — just capture what the model actually generates.

Upload to TLJH next to sft_checkpoint/ and datasets_v2/ and run:
    python eval_raw_capture.py

Results saved to eval_raw_capture/ with one .txt per dataset + summary.
"""
...
MAX_SEQ_LENGTH = 4096
MAX_NEW_TOKENS = 8192          # 2x previous — let the model finish
TEMPERATURE = 0.1
DATASET_DIR = "./datasets_v2"
NUM_DATASETS = 15              # Enough to see patterns
MIN_SUPPORT = 3
MAX_SIZE = 3
SEED = 42
OUTPUT_DIR = "./eval_raw_capture"

# ═══════════════════════════════════════════════════════════════════
# GPU setup
# ═══════════════════════════════════════════════════════════════════
print("=" * 70)
print("  RAW CAPTURE EVALUATION")
print("=" * 70)
...
def run_inference(csv_text, min_support):
    """Run inference and return raw output + token stats."""
    messages = [
    ...
    return {
        ...
        "hit_limit": new_tokens >= MAX_NEW_TOKENS,
...
def compute_f1(apriori_sets, model_items):
    ...
    for rec in model_items:
        if isinstance(rec, dict) and "itemset" in rec:
            ...
            if isinstance(itemset, list) and itemset:
                mod_c.add(frozenset(str(x).strip().lower() for x in itemset))
    tp = apr_c & mod_c
    p = len(tp) / len(mod_c) if mod_c else 0.0
    r = len(tp) / len(apr_c) if apr_c else 0.0
    f1 = 2*p*r/(p+r) if (p+r) > 0 else 0.0
    
    # Check hallucination (items not in any dataset row)
    return {
...
print(f"\n{'='*70}")
print(f"  Running with max_new_tokens={MAX_NEW_TOKENS}")
print(f"{'='*70}")
...
for i, csv_path in enumerate(selected, 1):
    ...
    try:
        ...
        metrics = compute_f1(apriori_sets, parsed_items)
        
        # Status line
        limit_warn = " ⚠️ HIT LIMIT" if inf["hit_limit"] else ""
        print(f"   Tokens: {inf['prompt_tokens']}→{inf['new_tokens']} | {inf['time_s']}s{limit_warn}")
        print(f"   Think={has_think} | JSON={parse_ok} ({len(parsed_items)} items) | F1={metrics['f1']:.0%} (P={metrics['precision']:.0%} R={metrics['recall']:.0%})")
...
if ok:
    ...
    print(f"    Parse rate:    {parse_rate:.0%}")
    print(f"    Think rate:    {think_rate:.0%}")
    print(f"    Hit token limit: {hit_limit}/{n}")
    print(f"    Avg tokens:    {sum(r['new_tokens'] for r in ok)/n:.0f}")
    ...
    for label, bucket in [("Small (<60 cells)", small), ("Medium (60-120)", medium), ("Large (120+)", large)]:
        if bucket:
            ...
            print(f"    {label:.<25} F1={bf1:.0%}  Parse={bparse:.0%}  n={len(bucket)}")
...
print(f"   summary.json with all metrics")
print(f"\n{'='*70}")
print(f"  DONE — check the .txt files to see exactly what the model generates")
print(f"{'='*70}")
